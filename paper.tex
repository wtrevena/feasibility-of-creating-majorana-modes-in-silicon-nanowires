...
\author{W.~T.~Trevena\thanks{Analysis, verification, and manuscript preparation were
assisted by AI agents (Claude, Anthropic), including three adversarial numerical-review
rounds documented in the repository's corrections log. All quantitative claims
regenerate from committed code.}}
\date{June 2026 --- draft v1}
\begin{document}
\maketitle

\begin{abstract}
We assess the feasibility of Majorana zero modes (MZMs) in silicon within a validated
Bogoliubov--de Gennes framework subjected to three adversarial review rounds.
...
physics---where we prove an exact two-band equivalence for smooth interfaces and show
that each single-atomic interface step acts as a near-$\pi$ Josephson junction
($\Delta\varphi = 2k_0 a/4 \approx 0.85\pi$) whose bound state caps the topological gap
at $\sim$18\% from the very first step. For the valence band we derive the gate-field
...
raising $g_z$ to 2.4--4.2. Field-orientation maps then select the viable designs: a
thick boron-doped superconducting-silicon (Si:B) parent with tilted field supports
10--19$\ueV$ using only measured Si:B critical fields, and a 10--20\,nm Si:B film is
orbitally consistent with Pauli limiting, supporting 21--27$\ueV$; an Al-film parent
would support 34--55$\ueV$ (metallization-renormalized) if a hard-gap Al/Si-hole
interface existed. Transport calculations show our spectral disorder thresholds are
...
pins every exact claim. All caricature choices (GL/Zeeman-linear parent gaps, iid
onsite disorder with correlation length $dx$, hard-wall LK geometry) are stated where
used.
...
Micromagnet arrays \cite{kjaergaard,turcotte} reach this scale in principle, and a 2025
InAs experiment measured synthetic $\alpha = 0.022$\,eV\,nm \cite{micromagnet2025},
so ``engineered-electron silicon'' is marginal rather than absurd---but the valley
physics below stacks on top of it.
...
pairing $\pm\Delta$ (verified to $3\times10^{-10}\ueV$ for valley-scalar and
...
\textbf{Step junctions.} Real interfaces are stepped, and each single-atomic step
shifts the VO phase by $\Delta\varphi = 2k_0(a/4) \approx 0.85\pi$---accidentally close
to $\pi$ in silicon. A controlled comparison (same-sign staircase vs sign-randomized
...
10--19$\ueV$ at fields tilted toward out-of-plane (riding $g_z$); the in-plane
...
models, iid disorder), and that this work's verification relied on documented
adversarial AI review rounds; the corrections log accompanies the code.

\section*{Repository}
All results regenerate from the committed code (\texttt{run\_analysis.py},
\texttt{transport.py}, \texttt{transport\_valley.py}, \texttt{lk\_holes.py},
\texttt{qp\_poisoning.py}, \texttt{tests.py}); figures and the numerical ledger are
under \texttt{output/}.

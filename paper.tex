...
Conduction-band wires face three independent, quantified obstructions: spin--orbit coupling (best
intrinsic topological gap $\sim$1.5$\ueV$; engineered SOC of $\alpha \gtrsim 0.02$\,eV\,\AA{}
required), a $g\!=\!2$ ceiling that, within our single-channel model, prevents induced
gaps above $116\ueV$ from reaching the topological phase at attainable fields (quantifying the optimal-coupling rule of Cole \emph{et al.}), and valley
...
covariation of $(\alpha, \hat g, m^*)$ from four- and six-band $k\cdot p$ fin
models with self-consistent tri-gate electrostatics: in the [100] models the
field that builds direct-Rashba SOC suppresses the wire-axis $g$ below 1.1,
but the long-standing factor-of-two gap to measured FinFET $g$-tensors is
resolved as predominantly a \emph{channel-orientation} effect --- all
benchmark devices are [110], where the model gives a gate-stable
$g_{x'} = 1.4$--$1.7$, $m^* \approx 0.20$, $\alpha$ up to 0.075\,eV\,\AA,
and the harmful $\alpha$--$g$ covariation largely lifts. Field-orientation maps then select the candidate designs. We label each input
...
\emph{conditional} feasibility result --- a hypothesis-generating design study
whose status two standard measurements would largely settle --- not an
established platform. Four realism controls sharpen these numbers: a dynamic tunneling self-energy
lowers the center-point gaps to 7.6 / 14.9 / 24.2$\ueV$ (measured-Si:B /
Pauli-Si:B$^\dagger$ / Al$^\dagger$), validating the static-renormalized
caricature directionally; orbital magnetic coupling is negligible at fin
widths ($<0.05\%$); the step-junction mechanism survives pairing-channel
mixing in the conservative direction; and correlated electrostatic disorder
is $4$--$8\times$ harsher than the iid baseline at matched RMS, making gate
smoothness the binding fabrication constraint. Transport calculations show
our spectral disorder thresholds are conservative, identify an abrupt
class-D breakdown mode, and warn that step-disordered wires become Anderson
insulators whose hard-looking transport gaps mask degraded local
protection. Quasiparticle poisoning restricts the measured-field all-silicon stack to
detection (not parity-coherent) experiments unless the parent gap is engineered. Two measurements would do the most to settle the platform: the parallel
critical field of thin Si:B films, and the wire-axis hole $g$-tensor in device
geometry.
\end{abstract}
...
3--30\,MV/m) keeps $g_x \in [0.4, 1.1]$, versus measured
$g_{xx} \approx 1.9$--2.3 \cite{geyer,camenzind}. The six-band $k\cdot p$ +
Poisson upgrade (Supplement S10) resolves this discrepancy: the split-off
band moves $g_x$ \emph{down} (to $[0.04, 0.93]$; its envelope weight is
negligible, consistent with Ref.~\cite{venitucci}), realistic tri-gate
electrostatics is a $\pm25\%$ correction whose sign depends on gate
geometry, and the dominant effect is \emph{channel orientation}: every
benchmark device is [110]-oriented while the model above is [100]. Rotating
to [110] gives a tight, nearly gate-independent $g_{x'} = 1.4$--$1.7$
across all geometries ($\sim$60\% of the gap; the remainder has the size
and sign expected from electrostatic asymmetry and 0.1--0.2\% process
strain \cite{venitucci}), with favorable side effects --- $m^* \approx
0.20$, $\alpha$ up to 0.075\,eV\,\AA{} --- and the harmful $\alpha$--$g_x$
covariation largely absent. The platform numbers below are carried under
both the (superseded) [100] tensors and the [110] re-evaluation of
Supplement S10.3.
...
The six-band [110] re-evaluation (Supplement S10.3) changes the design
geometry more than the magnitudes: with $g_{x'} \approx 1.6$ usable
\emph{in-plane along the wire}, the film parents no longer need the
orientation compromise (Al: 26--32$\ueV$ static renormalized, 17--18
dynamic; Pauli film$^\dagger$: 17--19 static renormalized, 13 dynamic),
while measured-field Si:B drops to 3.8$\ueV$ under the dynamic self-energy
--- below the Tier-S threshold of Table~\ref{t:tiers}, so the
detection-class all-silicon experiment now itself requires the
(hypothetical) Pauli-limited film.
...
\section{Realism controls, master assumption table, and operational
criteria}\label{sec:controls}
Review round 5 added four controls (Supplement S9; \texttt{realism.py},
...
The six-band [110] re-evaluation (Supplement S10.3, performed after this
table was frozen) supersedes the LK-tensor rows toward lighter mass
($m^* \approx 0.20$), larger $\alpha$ (up to 0.075\,eV\,\AA), in-plane
usable $g_{x'} \approx 1.6$, and the dynamic central estimates quoted in
Sec.~\ref{sec:holes}.

\input{drafts/usable_criterion}

...
\author{W.~T.~Trevena}
\date{June 2026 --- accompanies draft v4 (versions synchronized; see Data availability)}
\begin{document}
\maketitle
\tableofcontents
...
figures (i) field suppression enters through the parent,
...
used. The full dynamical self-energy is solved as a control in
Sec.~\ref{s:dynse}: it lowers the optimal gaps 24--30\% below the
static-renormalized values (which therefore act as upper bounds) and agrees
with the static model at matched effective coupling to $\lesssim 7\%$ ---
the caricature's direction and approximate magnitude are confirmed.
...
$\gamma_3 = 1.446$, $\kappa = -0.42$; parameter provenance: Hensel--Feher
cyclotron resonance via Winkler/Venitucci --- see
\texttt{drafts/kp6\_benchmarks.md}) with vertical field $E_z$; the
...
\section{Parent and disorder realism controls}\label{s:controls}
Four controls (review round 5) test the headline numbers against the
caricatures they rest on. All regenerate from \texttt{realism.py},
\texttt{orbital.py}, \texttt{pairing\_mix.py}, \texttt{morphology.py}
(ledgers \texttt{realism/orbital/pairing\_mix/morphology} in
\texttt{key\_numbers.json}).

\subsection{Dynamic tunneling self-energy}\label{s:dynse}
The static $\Delta$ is replaced by the full frequency-dependent BCS
self-energy $\Sigma(\omega) = -\Gamma(\omega + \Delta_p\tau_x)/
\sqrt{\Delta_p^2-\omega^2}$; quasiparticle energies solve
$\det[H(k)+\Sigma(\omega)-\omega]=0$ (batched eigenvalue crossing scan;
$\alpha$, $g$, $m^*$ renormalization is automatic through the
$\omega$-linear term). Validation: static limit ($\Delta_p \to \infty$,
$\Sigma \to -\Gamma\tau_x$) matches \texttt{bulk\_gap\_ueV} to 0.07\%;
$(n_k, n_\omega)$ doubling changes results by 0.001\%. The topological
criterion is exact at $\omega = 0$: $E_Z^2 > \Gamma^2 + \mu^2$.

\begin{table}[h]\centering\small
\caption{Dynamic topological gap ($\ueV$) vs tunnel coupling $\Gamma$ at the
three parent operating points ($\mu = 0$, empirical-center hole tensor).}
\label{t:dynse}
\begin{tabular}{lccccccccc|ccc}
\toprule
$\Gamma$ ($\ueV$) & 3 & 6 & 10 & 15 & 20 & 30 & 45 & 70 & 100 &
best ($\Gamma^*$) & static@$\Delta_{ind}(\Gamma^*)$ & static bare\\
\midrule
Si:B meas.\ (0.33\,T) & 2.7 & 5.0 & \textbf{7.6} & 3.9 & 0.6 & 0 & 0 & 0 & 0
 & 7.6 (10) & 7.4 & 11.0\\
Si:B Pauli (1\,T) & 2.7 & 5.0 & 7.6 & 10.4 & 12.7 & \textbf{14.9} & 7.3 & 0 & 0
 & 14.9 (30) & 15.2 & 30.6\\
Al (1.7\,T) & 2.7 & 5.0 & 7.9 & 11.1 & 13.9 & 18.7 & \textbf{24.2} & 15.3 & 2.9
 & 24.2 (45) & 22.9 & 50.4\\
\bottomrule
\end{tabular}
\end{table}

The dynamic optimum is $1.4$--$2.1\times$ below the static \emph{bare}
numbers, sits $24$--$30\%$ below the static-$Z$ \emph{renormalized}
caricature (10.1 / 19.7 / 34.4), and agrees with the static model evaluated
at the matched effective induced gap
$\Gamma^*\Delta_p/(\Gamma^*+\Delta_p)$ to $\lesssim 7\%$ --- the static-$Z$
treatment is directionally validated, and all main-text gap ranges now carry
the dynamic correction as the better central estimate. Dynes broadening
adds a temperature floor rather than a gap penalty: subgap DOS fraction
$\gamma_D/\Delta_p$ dominates the thermal quasiparticle budget below
37--71\,mK (Si:B, $\gamma_D/\Delta_p = 10^{-4}$--$10^{-2}$) and
71--136\,mK (Al), so 50\,mK operation requires
$\gamma_D/\Delta_p \lesssim 10^{-3}$ for Si:B parents. Correlated
parent-gap inhomogeneity ($\Delta(x)$, 50\,nm correlation) is benign at the
10--25\% level (median gap 88--97\% of clean) and halves the gap at 50\%.

\subsection{Orbital magnetic coupling}\label{s:orbital}
Peierls phases of the perpendicular field component on a 2D strip
(validated against \texttt{build\_wire\_2d} to $3\times10^{-13}\ueV$ at
$B_\perp = 0$; exact Hermiticity and PHS). At the preferred tilted Si:B
operating point ($B = 0.33$\,T, $\theta \approx 39^\circ$,
$\Delta_{ind} = 11.3\ueV$) the orbital gap suppression is
\textbf{0.02--0.04\%} for fin widths 7--16\,nm and 0.3\% at 40\,nm
(scaling $\propto W^{1.5}B_\perp^2$; the 10\% level extrapolates to
$W \sim$ several hundred nm). Even a full 1\,T out-of-plane field costs
only 0.3 / 1.4 / 8.9\% at $W = 10/20/40$\,nm, with the zero mode and end
localization intact. With $\mu$ at the lowest subband edge the transverse
subband spacing (0.7--6\,meV) dwarfs the orbital matrix elements; omitting
orbital effects does not alter any orientation-map conclusion at the
$\ueV$ scale.

\subsection{Pairing-channel mixing}\label{s:mix}
$D(\eta) = \Delta[\sqrt{1-\eta^2}\,(\nx\otimes i\sigma_y) +
\eta\,(\nu_0\otimes i\sigma_y)]$ interpolates from the physical
inter-valley channel to pure intra-valley pairing at preserved total
weight ($\eta = 0$ reproduces the production builder to
$2\times10^{-13}\ueV$). The dilution intuition is \emph{wrong} in the
direction favorable to our claims: the valley rotation that removes a VO
phase step maps $\nu_0\otimes i\sigma_y \to e^{-i\phi\nz}\otimes
i\sigma_y$, so the intra channel inherits the step as a pairing-phase
junction --- the single-step bound state deepens monotonically
(4.03 $\to$ 3.12 $\to$ 1.68$\ueV$ at $\eta = 0, 0.1, 0.3$; $2\times$ by
$\eta^* \approx 0.24$; near-zero junction modes at $\eta = 1$). Staircase
\emph{ensembles} conversely heal with admixture (median $E_2$:
0.57/1.8/4.5$\ueV$ at $\eta = 0/0.1/0.3$; the phase-blind intra channel
supplies a pairing floor), so the $\eta = 0$ ensemble gaps quoted in the
main text are conservative. Physically $\eta \ll \eta^*$: an intra-valley
pair carries momentum $2k_0$, which a momentum-conserving uniform parent
cannot supply; intra-valley pairing requires interface-roughness Fourier
weight at $2k_0$ --- the same small parameter that generates the VO
coupling itself.

\subsection{Step morphology and correlated electrostatic disorder}
\label{s:morph}
\textbf{Morphology} (gamma-distributed terraces, shape $k = 4$; miscut
$\theta_m$ sets the mean terrace $s = (a/4)/\tan\theta_m$; wire at angle
$\chi$ to step edges; generator validated against the fig.~9 pipeline to
$5\times10^{-7}\ueV$ per seed). Reported as $p_5/p_{50}/p_{95}$ of $E_2$
($\ueV$) and the failure fraction $f_{<1} = P(E_2 < 1\ueV)$:
miscut scan ($\chi = 90^\circ$): $0.05^\circ$: 0.5/2.0/3.1, $f_{<1}=0.25$;
$0.1^\circ$: 0.3/1.1/2.8, 0.33; $0.2^\circ$: $\sim$0/0.005/0.02, 1.00;
$0.5^\circ$: 0/0/0.001, 1.00. Orientation ($\theta_m = 0.1^\circ$):
$\chi = 90/45/20/10/5^\circ$ medians 1.5/1.7/2.2/3.2/3.8 with $f_{<1}$
falling $0.33 \to 0$. Finite ramp widths $\le 10$\,nm and VO-amplitude
dips (to 30\% over 3\,nm) shift medians by only 10--15\% (the latter
strictly worsening); double-step bunches ($\Delta\varphi \equiv -0.3\pi$)
give 2.7/5.9/10.2 with $f_{<1} = 0$ at the same height budget. Rare-event
tails, not medians, are binding: at realistic 0.05--$0.1^\circ$ miscuts
25--33\% of realizations fail even where the median looks viable ---
yield is the constraint. Design rule: wires along step edges
($\chi \lesssim 10^\circ$) on $\le 0.05^\circ$ substrates, or
deliberately step-bunched templates.

\textbf{Correlated disorder} (Gaussian random field $\mu(x)$, correlation
length $\lambda_c$, vs iid at matched RMS; hole operating point, clean
$E_2 = 31.3\ueV$): at 50$\ueV$ RMS the median collapses to
6.8/4.8/3.5$\ueV$ for $\lambda_c = 25/50/100$\,nm while the iid baseline
retains 28.9$\ueV$ --- \emph{the iid model overstates robustness by
$4$--$8\times$ in gap at fixed RMS}, making gate-potential smoothness at
the few-tens-of-$\ueV$ level the binding fabrication requirement.
$\alpha$ fluctuations (20\%, $\lambda_c = 50$\,nm) are negligible
(median 29.6); $g$ fluctuations (20\%) cost $\sim$12\% (median 27.5).
(Caveat: at RMS $= 100\ueV > E_Z = 64\ueV$ the wire fragments and $E_2$
no longer measures a topological gap; 10 seeds per cell.)

\section{Six-band $k\cdot p$ + Poisson: resolving the $g_x$ discrepancy}
\label{s:kp6}
The decisive theory upgrade demanded in review round 5
(\texttt{kp6\_holes.py}, \texttt{kp6\_110.py}, \texttt{poisson2d.py},
\texttt{kp6\_sc.py}; ledger tags \texttt{kp6/kp6\_110/poisson2d/kp6\_sc/
platform110}; literature anchors in \texttt{drafts/kp6\_benchmarks.md}).

\subsection{Model and validation}
Six-band $\Gamma_8\oplus\Gamma_7$ LKBP Hamiltonian (same $\gamma_i$, the
standard Chao--Chuang/Winkler coupling blocks, $\Delta_{so} = 44$\,meV;
Zeeman $H_Z = \mu_B(g_L^h\,\mathbf{B\cdot L} + g_s^h\,\mathbf{B\cdot S})$
with $g_s^h = -2$, $g_L^h = 3\kappa + 1$, which reproduces $+2\kappa\mu_B
\mathbf{B\cdot J}$ in $\Gamma_8$; identical Peierls orbital treatment as the
four-band code). Finite differences do \emph{not} converge for the 6-band
problem at affordable grids (the $\Gamma_8$--$\Gamma_7$ mixing amplifies the
$O(d^2)$ error to 45\% in $\alpha, g_x$ at $11\times13$); production uses a
hard-wall sine (Galerkin) basis, with the FD mode retained as the
conventions bridge. Validation: bulk $k = 0$ exact; [100] HH mass
$1/(\gamma_1 - 2\gamma_2)$ to $2\times10^{-15}$; closed-form bulk branches
along [001]/[111] to $3\times10^{-15}$; $\Delta_{so}\to\infty$ reproduces
\texttt{lk\_holes.extract} to 0.006\%; Kramers $2.5\times10^{-12}$;
grid drift $<3\%$. The [110]-channel rotation (kinetic-tensor rotation,
identical pipeline) is validated against the unrotated model at rotated
momenta to $10^{-15}$ and coincides with it in the isotropic
$\gamma_2 = \gamma_3$ limit to $10^{-6}$.

\subsection{Three findings}
\textbf{(i) The split-off band moves $g_x$ the wrong way.} At the
production fin ($10\times12$\,nm, $E_z = 10$\,MV/m, [100] channel) the
6-band parameters are $m^* = 0.85$, $\alpha = 0.024$\,eV\,\AA,
$g = (0.62, 1.03, 4.15)$ vs four-band $(0.44, 0.048, (1.04, 1.99, 3.40))$;
the $\Delta_{so}$ sweep is monotone (\,$g_x$: 1.06 at $\Delta_{so}\to\infty$
$\to$ 0.62 at 44\,meV), and the seven-geometry bracket drops to
$g_x \in [0.04, 0.93]$. Consistent with Venitucci \emph{et al.}: split-off
envelope weight is negligible in the ground doublet; level repulsion makes
the channel heavier and transfers $g$-weight to $g_z$ (which reaches
4.2--4.8, consistent with the published HH-like $g_z$ peak of 4.66--5).

\textbf{(ii) The discrepancy was predominantly an axes mismatch: all
benchmark devices are [110]-channel.} Rotating the channel to [110] (the
orientation of every measured device --- Geyer, Camenzind, Voisin, Crippa)
with everything else identical gives a tight, nearly field-independent
$g_{x'} = 1.43$--$1.66$ across the full $E_z$ sweep and all seven
geometries, closing $\sim$60\% of the gap to the measured along-fin
1.9--2.3; $g_{y'} = 2.0$--$2.2$ at low field falls inside the measured
window, and $g_z$ drops toward the measured 1.5. Favorable side effects:
$m^* \approx 0.20$ (vs 0.85), $\alpha$ up to 0.075\,eV\,\AA, and the
harmful $\alpha$--$g_x$ covariation of the [100] model largely lifts
($g_{x'}$ varies by only 15\% over the whole gate range). The residual
0.3--0.6 shortfall has the size and sign the literature attributes to
inhomogeneous electrostatics and 0.1--0.2\% process strain.

\textbf{(iii) Realistic electrostatics is a $\pm25\%$, geometry-signed
correction.} The self-consistent tri-gate solution (validated Poisson +
SCF, 21 iterations at production parameters; uniform-field control
reproduces \texttt{extract6} to 0.00\%) shifts $g_x$ by $-23\%$ (symmetric
tri-gate, lateral squeezing) to $+24\%$ (top gate only) at matched
density-weighted field; lateral asymmetry ($\Delta V = 0.2$\,V between
sidewalls) raises $g_x$ by $+31\%$ and $\alpha$ by $+56\%$ together while
rotating the SOC axis toward $-\hat z$. Electrostatics alone closes at most
$\sim$0.1--0.2 of the [100] gap --- orientation is the leading correction.

\subsection{Platform re-evaluation at [110] parameters}
\label{s:plat110}
The frozen [100]/static master assumption table of earlier drafts is
retained for provenance:

\input{drafts/master_table}

With $(m^*, \alpha, g_{x'}) = (0.19, 0.052, 1.55)$ [$E_z = 10$\,MV/m] and
$(0.20, 0.073, 1.66)$ [30\,MV/m], the \emph{in-plane along-wire} field
direction --- the only direction film parents tolerate --- becomes usable
(\texttt{platform110}; $10\times10\times20$ optimizer grids):

\begin{table}[h]\centering\small
\caption{Hole-platform operating points with six-band [110] parameters.
``dyn.''\ = dynamic self-energy optimum (Sec.~\ref{s:dynse} method) at the
static operating field; static bare/renormalized as before. In-plane
along-wire field except the $g_z$ rows (tilted, thick-Si:B parent only).}
\label{t:plat110}
\begin{tabular}{llccc}
\toprule
scenario & field dir. & static bare & static renorm. & dynamic
($\Gamma^*$)\\
\midrule
Si:B measured ($B_{c2}{=}0.4$\,T) & in-plane $x'$ & 8.5--8.9 & 8.1--8.7 &
3.8 (6)\\
Si:B measured, tilted & $g_z$ ride & 12.5--17.1 & 10.3--12.6 & ---\\
Si:B Pauli film$^{\dagger}$ & in-plane $x'$ & 21.0--23.3 & 16.6--18.7 &
12.5--13.0 (20)\\
Al film$^{\dagger}$ & in-plane $x'$ & 36.6--40.5 & 26.0--32.1 &
16.9--18.4 (30)\\
\bottomrule
\end{tabular}
\end{table}

Two consequences. First, the film-parent designs no longer require the
orientation compromise: the Al and thin-film Si:B scenarios operate
in-plane along the wire at $g_{x'} \approx 1.6$. Second, the honest central
estimates (dynamic column) sharpen the tier verdicts: measured-field Si:B
falls to 3.8$\ueV$ --- \emph{below} Tier S --- so detection-class
experiments on the all-silicon stack now require the (hypothetical)
Pauli-limited film, while the Al scenario sits at 17--18$\ueV$ dynamic.
All numbers remain hypothesis-generating: hard-wall cross-section, no
strain, and the [110] tensor still 0.3--0.6 short of measured $g_{x'}$.

\section{Device-level realism: tuning, transport observables, yields, and
charge-trap calibration}\label{s:device}
(\texttt{r6\_scenarios.py}, \texttt{r6\_transport.py}, \texttt{r6\_morph2.py},
\texttt{r6\_multiband.py}; ledger tags \texttt{r6\_*}.)

\subsection{Tilted-field dynamic self-energy and tuning tolerances}
\label{s:tilt}
The tilted measured-Si:B case, previously static-only (12.5--17.1$\ueV$
bare), re-evaluated with the full $\Sigma(\omega)$ and scalar
$g_{\rm eff}(\theta) = [(g_{x'}\cos\theta)^2 + (g_z\sin\theta)^2]^{1/2}$
(exact for the [110] tensors, whose SOC axis $\pm\hat y'$ is perpendicular
to the $x'$--$z$ tilt plane): the optimum is fully out-of-plane and gives
\textbf{8.4$\ueV$} ([110], $E_z = 10$\,MV/m; $\Gamma^* = 10\ueV$,
$B^* = 0.27$\,T) to \textbf{10.5$\ueV$} ($E_z = 30$; $\Gamma^* = 15$,
$B^* = 0.32$\,T), uniformly 69\% of the matched-$\Delta_{ind}$ static
value. The static 12.5--17.1$\ueV$ numbers are superseded. (Footnote: the
\texttt{platform110} in-plane dynamic entries were evaluated at the
static-optimal $B$; re-optimizing $B$ raises them $\sim$25--40\%, e.g.\
measured-Si:B in-plane 3.8 $\to$ 5.3$\ueV$ at 0.27\,T --- the ledger
values are conservative.) Tuning tolerances at the three dynamic optima
(window where the gap stays $\ge 50\%$): $\mu$: $\pm14$ /$\pm28$ /
$>\!\pm30\ueV$ (measured-Si:B / Pauli / Al); $B$: $\ge 0.13$ / $\ge 0.6$ /
$\ge 0.7$\,T; $\Gamma$: factor $\ge 3$ everywhere. The one tight margin is
field overshoot at measured-Si:B (0.02\,T below $B_{c2}$ at the
static-optimal bias) --- an argument for biasing at 0.27--0.34\,T.

\subsection{Hole-scenario transport, finite-$T$ spectroscopy, and the
failure-mode library}\label{s:holetrans}
The RGF validation gauntlet passes at all three hole operating points
([110] Si:B-Pauli, [110] Al, legacy empirical): clean transport gap =
analytic bulk gap to $\le 2\%$ (23.5/37.1/31.3 vs 23.3/36.6/30.7$\ueV$),
$T(E{=}0) \le 10^{-4}$ through 2\,$\mu$m, and $\mathrm{sign}\det r$ flips
$+1 \to -1$ across $B^*$ ($k_{so}dx \le 0.05$). Thermal/Lorentzian
convolution at the Si:B-Pauli point: at 50\,mK ($\gamma = 0.5$--2$\ueV$)
the 10\%-onset underestimates the 23.5$\ueV$ gap by 30--40\% while the
half-rise overshoots to $\approx$27$\ueV$ (bracketing $\pm$35\%); at
100\,mK the gap is unresolvable; mild disorder ($W = 200\ueV$) already
adds genuine zero-energy leakage in 2 of 3 seeds --- apparent subgap
softness cannot by itself distinguish temperature from disorder.
Failure-mode library (end dots $-150/-250\ueV$, smooth 300$\ueV$
confinement, $W = 600\ueV$ worst-of-ten; each in trivial and topological
phases): at this single-channel $\mu = 0$ point no trivial generator
produces a near-zero mimic ($E_0 \ge 6.6\ueV$, $Q = +1$ throughout);
the dangerous mode is the \emph{false negative} --- smooth confinement on
the topological side hides a genuine zero mode from two-terminal
diagnostics ($Q$ reads $+1$); and $W = 600\ueV$ destroys protection while
closed-spectrum states linger near zero. No single scalar suffices: the
discriminating pair is $\mathrm{sign}\det r$ co-read with $E_2$/transport
gap. Scope: all two-terminal local NS transport; nonlocal (end-to-end)
conductance --- the modern experimental protocol --- is not implemented
here.

\subsection{Wafer-spec yields and the step-junction's robustness}
\label{s:yield}
Converting step ensembles to fab-facing yields (yield $\equiv$
$P(E_2 \ge 4\ueV)$, the single-step ceiling; 40 realizations/cell):
premium $\pm0.05^\circ$ wafers yield 2.5\% (unaligned $\chi = 90^\circ$),
20\% ($\chi = 30^\circ$), \textbf{68\%} ($\chi = 10^\circ$); standard
$\pm0.1^\circ$: 5\%/5\%/50\%. Alignment, not wafer grade, dominates. The
junction itself survives realistic smoothing and valley splitting
combined: over $\lambda \in [40, 300]\ueV \times w \in [0, 20]$\,nm the
single-step ratio $E_2^{\rm step}/E_2^{\rm clean}$ stays 0.18--0.28
(the junction responds to the total phase jump, compared against the
induced coherence length, not the ramp width); small splitting
($\lambda = 40\ueV$) additionally caps the \emph{clean} gap at 15$\ueV$
through the second valley band.

\subsection{Charge-trap calibration of correlated disorder}\label{s:trap}
The GRF disorder levels are calibrated against a physical source: one
$\pm e$ trap at the Si/oxide interface, screened self-consistently
(\texttt{poisson2d} SCF at $n_l = 2\times10^7$/m), shifts the channel
centroid potential by $A_{\rm trap} \approx 14$\,meV (8.8--20\,meV over
interface sites). Good-oxide densities ($10^{10}$--$10^{11}$\,cm$^{-2}$)
then produce 3--14\,meV RMS $\mu(x)$ disorder --- \emph{two orders of
magnitude beyond the fatal 100-$\ueV$ level}, destroying the topological
phase outright in 10/10 seeds at every density studied. The 25--50$\ueV$
RMS levels of Sec.~\ref{s:controls} correspond to $\lesssim 10^7$
cm$^{-2}$: an effectively trap-free wire. Even granting the proxy's
caveats (2D cross-section transfer, toy screening density --- metallic
screening could reduce $A_{\rm trap}$ by an order of magnitude) the
conclusion is robust: \textbf{oxide-interface charge, not step morphology
or parent choice, is the binding materials constraint} for any MOS-style
Si Majorana device; the alternatives are dramatically better interfaces,
remote-doping-free Si/SiGe-style channels with large setback, or
metallic-density screening.

\subsection{Multi-subband occupancy and spatially resolved coupling}
\label{s:multi}
Multi-subband strip with [110] parameters ($W = 15$--25\,nm): the
topological windows at the 2nd and 3rd transverse subband bottoms are
indistinguishable from the 1st (widths 59--72$\ueV$ vs the 82$\ueV$ 1D
prediction; gaps 22--27$\ueV$ in every window; subband spacing
$\ge 60\times$ the Zeeman scale) --- single-subband operation is not
required; the requirement remains $\mu$ within $\sim\pm35\ueV$ of
\emph{some} subband edge. (Caveat: effective single-band occupancy
control, not full LK multiband realism.) Spatially resolved coupling
$\Gamma(x)$ in the static limit (local $\Delta_{ind}(x)$, $Z(x)$,
$\alpha(x)$, $g(x)$): 25--50\% RMS fluctuations cost only 5--14\% median
gap (worst of 32: 20\%) --- pairing-channel disorder creates no potential
wells and is partially self-compensating; 50--150\,nm dead zones cost gap
$\propto$ lost coverage and bind no in-gap states; an uncovered 100-nm
end acts as a shortened wire (Majorana relocates to the film edge,
$E_0 = 0.050$ vs 0.047$\ueV$ control).
...
\texttt{convergence.py} (Sec.~\ref{s:conv}), \texttt{realism.py},
\texttt{orbital.py}, \texttt{pairing\_mix.py}, \texttt{morphology.py}
(Sec.~\ref{s:controls}), \texttt{kp6\_holes.py}, \texttt{kp6\_110.py},
\texttt{poisson2d.py}, \texttt{kp6\_sc.py}, \texttt{platform110.py}
(Sec.~\ref{s:kp6}), \texttt{r6\_scenarios.py}, \texttt{r6\_transport.py},
\texttt{r6\_morph2.py}, \texttt{r6\_multiband.py} (Sec.~\ref{s:device}),
...
scan checkpoints allow resuming every long run. The repository is public:
\url{https://github.com/wtrevena/feasibility-of-creating-majorana-modes-in-silicon-nanowires}. Archival packaging

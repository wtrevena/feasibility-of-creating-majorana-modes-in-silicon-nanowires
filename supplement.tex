...
The formula tracks the numerics to $\sim$10\% for
$\Delta\varphi \lesssim 0.5\pi$ and overestimates by $20$--$25\%$ near
$0.85\pi$ --- the expected accuracy for an atomically abrupt junction
...
\texttt{fig9} key numbers: gap intact at $q \le 4\times10^6$\,m$^{-1}$,
destroyed by $1.6\times10^7$); at 50\,nm step spacing the equivalent
...
(\texttt{output/data/convergence.json}). The observables are the clean
...
leaves its topological window for $E_v \gtrsim 225\ueV$; single-valley
...
\midrule
\multicolumn{5}{l}{$k$-grid $n_k$: $\le 0.02\%$ drift from 501 to 12001 at
...
gives $T_{\mathrm{eff}} \le 25$--$29$\,mK; the same criterion for Al
($\Delta_p \approx 180\ueV$ at 1.7\,T) gives 130\,mK. Since realistic
...
spin--orbit lengths $l_{so} = \hbar/(m^*\alpha) = 32$--$101$\,nm cover the
measured FinFET window. Limits: no strain, no cubic ($J^3$) Zeeman, no
